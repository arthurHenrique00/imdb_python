...
def extract_movie_deatails(movie_link):
    ...
    if movie_soup is not None:
        title = None
        date = None
        page_section = move_soup.find('section', attrs={'class': 'ipc-page-section'})
        ...
        if page_section is not None:
            ...
            if len(divs) > 1:
                ...
                plot_tag = movie_soup.find('span', attrs={'dat-testid': 'plot-xs_to_m'})
                plot_text = plot_tag.get_text().strip() if plot_tag else None

                with open('rating_movies.csv', mode='a', newline='', encoding='utf-8') as file:
                    movie_writer = csv.writer(file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

                    if all([title, date, rating, plot_text]):
                        print(title, date, plot_text)
...
def extract_movies(soup):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as exec:
        exec.map(extract_movie_deatails, movie_links)

def main():
    start_time = time.time()
    pop_movies_url = 'https://www.imdb.com/chart/moviemeter/?ref_=nv_mv_mpm'
    response = requests.get(pop_movies_url, headers=headers)
